...
public class PostService {
    ...
    public void addPost(Post p){
        jdbcTemplate.execute("INSERT INTO posts(name,text,threadid) VALUES ('"+ p.getName()+"','"
                +p.getText()+"','"+p.getThread()+"')");
    ...
    public List<Post> findPosts(Monitor monitor){
        ...
        String q = "SELECT id, name, text, threadID FROM posts WHERE " + (monitor.isCaseSensitive() ? "text" : "LOWER(text)") + " like '%" + query +"%'";

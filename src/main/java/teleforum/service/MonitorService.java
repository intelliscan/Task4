...
public class MonitorService {
    ...
    public void reset(){
        ...
        entityManager.createNativeQuery("CREATE TABLE monitor(id SERIAL, query VARCHAR(255), name VARCHAR(255), max_results INT, case_sensitive BIT)").executeUpdate();
    }
    ...
    public Monitor getMonitor(Integer id){
        return  entityManager.createQuery("SELECT m FROM Monitor m WHERE m.id = :id", Monitor.class).setParameter("id", id).getSingleResult();
